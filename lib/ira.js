...
function Ira(options) {
    ...
    var handler = ira._bind('_serverHandler')

    // Switch server protocol
    if (ira.spdy) {
        try {
            serv = require('spdy').createServer(https.Server, ira.spdy, handler)
        } catch (excp) { throw new Error('Run `npm install spdy` before') }
    }
...
Ira.include('define', function (prop, action) {
...
Ira.include('get', function get(path) {
    ...
    if (typeof(path) === 'object') {
        ...
        else return this[path.name][path.value.name][path.value.value]
    } 
